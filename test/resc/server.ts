...
app.get('/origin', (req: any, res: any) => {
	res.send(JSON.stringify({
		origin: req.ip
	}));
});

app.get('/user-agent', (req: any, res: any) => {
	res.send(JSON.stringify({
...
['delete', 'get', 'head', 'options'].map((method: any) => {
	(app as any)[method](`/${method}`, (req: any, res: any) => {
		res.status(204);
		res.end();
	});
});

["post", "patch", "put"].map((method: any) => {
	(app as any)[method](`/${method}`, (req: any, res: any) => {
		res.json(req.body);
		res.end();
	});
